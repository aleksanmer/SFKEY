...
namespace EZGUI
{
    ...
    internal class Drawer
    {
        ...
        #region Methods
        public static void DrawCircle(Vector2 Center, float Radius, ColorBGRA color, short Sides)
        {

            float Angle = (float)((Math.PI / 180f) * (360.0f / (float)Sides));

            float Cos = (float)Math.Cos((double)Angle);
            float Sin = (float)Math.Sin((double)Angle);

            Vector2 vec = new Vector2(Radius, 0);

            for (short i = 0; i < Sides; ++i)
            {
                Vector2 rot = new Vector2(Cos * vec.X - Sin * vec.Y, Sin * vec.X + Cos * vec.Y);
                rot += Center;
                vec += Center;
                DrawLine(vec.X, vec.Y, rot.X, rot.Y, 3, color);
                vec = rot - Center;
            }
        }
